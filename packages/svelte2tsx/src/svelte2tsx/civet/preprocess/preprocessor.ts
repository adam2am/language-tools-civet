...
import { DEBUG_FLAGS } from '../mapping/dense/constants';
import { offsetToPosition } from '../util/string';

const civetPreprocessorDebug = DEBUG_FLAGS.PREPROCESS;
...
export function preprocessCivet(
  ...
  // --- Performance Logging Init ---
  let totalCompileTime = 0;
  let totalNormalizeTime = 0;
  const tPreprocessStart = DEBUG_FLAGS.BENCHMARK ? performance.now() : 0;
...
${snippet}`);
      ...
      // ---------------- Per-line extra indent capture ----------------
      // After common-indent removal, remaining leading whitespace on each
      // line represents the "extra" indent that varied per-line. We capture
      // that so the map chainer can adjust column offsets precisely.
      const extraIndentPerLine = content.split('\n').map((ln) => {
        const m = ln.match(/^\s*/);
        return m ? m[0].length : 0;
      });
      const hasExtraIndent = extraIndentPerLine.some((n) => n !== 0);
...
${content}`);
      ...
      const tCompileStart = DEBUG_FLAGS.BENCHMARK ? performance.now() : 0;
      ...
      if (DEBUG_FLAGS.BENCHMARK) {
          totalCompileTime += performance.now() - tCompileStart;
      }
...
${tsCode}`);
      ...
      const tNormalizeStart = DEBUG_FLAGS.BENCHMARK ? performance.now() : 0;
      ...
      if (DEBUG_FLAGS.BENCHMARK) {
        totalNormalizeTime += performance.now() - tNormalizeStart;
      }
      ...
      const blockData: BlockInfo = {
        ...
        sourceIndent: {
            ...
            ...(hasExtraIndent ? { perLineLengths: extraIndentPerLine } : {})
        },
      };
  ...
  if (DEBUG_FLAGS.BENCHMARK) {
    const tPreprocessEnd = performance.now();
    const totalPreprocessTime = tPreprocessEnd - tPreprocessStart;
    const otherTime = totalPreprocessTime - totalCompileTime - totalNormalizeTime;

    console.log(`\n--- [preprocessCivet] Performance Breakdown ---`);
    console.log(`Total Time         : ${totalPreprocessTime.toFixed(1)} ms`);
    console.log(`  - Civet Compile    : ${totalCompileTime.toFixed(1)} ms`);
    console.log(`  - Map Normalization: ${totalNormalizeTime.toFixed(1)} ms`);
    console.log(`  - Other (overhead) : ${otherTime.toFixed(1)} ms`);
    console.log(`---------------------------------------------`);
  }
